...
- Spring Boot JPA + Oracle + Thymeleaf + React
    ...
    - H2 - Oracle, MySQL 등과 달리 인메모리DB. SpringBoot에서 자동으로 실행해주는 DB
...
- Spring Boot 프로젝트 생성
    1. 명령 팔레트 시작 (Ctrl + Shift + P) : SpringInitializr: Create a Gradle (메이븐보다 속도 빠르고 크게 다른점 없음) Project
    ...
    8. Dependency
        ...
        - Lombok : 어노테이션 등 편리하게 해주는 플러그인 (Getter, Setter 자동생성?)
...
- Spring Boot Backboard project (왼쪽에 ON 버튼 모양)
...
1. 기본 실행
    1. resource/application.properties
        ...
        logging.level.root=info     # 로그 출력 레벨
        logging.file.name=/backboard.log    # 로그 파일 위치
        ```
...
2. DB 연동
    ...
    2. application.properties에 H2 관련 설정 추가
        ...
        ```
    3. http://localhost:8100/h2-console 접속

        <img src="./image/sb012.png" width="450">
    ...
    9. 단위테스트
        1. build.gradle에 JUnit 의존성 추가
            ...
            ```gradle
            // JUnit 단위테스트
	        tesetImplementation 'org.junit.jupiter:junit-jupiter'
            ```

        2. INSERT 단위테스트
        3. test/../backboard/BackboardApplicationTests.java에 단위테스트 메서드 작성

            <img src='./image/sb013.png' width="600">
...
2. 개발 계속
    ...
    8. /templates/board_list.html 생성
        - thymeleaf 문법 적용
            ...
            <img src="./image/sb014.png" width="600">

    9. 상세 페이지 작업
        - Service, Controller 메서드 작업
    10. 상세 페이지에 댓글 저장 기능 추가
        - board_detail.html에 댓글 저장 폼양식 추기
...
3. thymeleaf 문법
    ...
    - ~{} : Fragment포함 표현식. 템플릿 Fragment를 사용
...
5. 전체 HTML에 디자인 적용
    ...
    3. board_list.html에 적용
       ...
       https://github.com/user-attachments/assets/684d2dd7-4aac-4989-8bdf-548e84ea736c
...
1. DB 연동
    ...
    2. Spring Boot Validation 기능 추가 : 입력 검증
        ...
        ```
        - Annotaion으로 검증 수행
            - @Size, @NotNull, @NotEmpty, @Past(과거날짜만 가능), @Future(미래날짜만 가능)
    ...
    6. board_deatil.html에 댓글 입력 검증 태그 추가
        - ReplyController의 PostMapping 메서드에 ReplyForm을 파라미터로 추가
        - BoardController의 GetMapping 메서드에 ReplyForm을 파라미터로 추가
    7. 검증영역 태그를 valid_errors 템플릿 생성
...
4. Paging : 대량 데이터 로드시 속도 개선
    ...
    - board_list.html에 페이징 컨트롤 추가
        ...
        <img src="./image/sb015.png" width="600">
...
1. 게시판 작업
    ...
    2. 게시글 최신 글부터 나오도록 정렬
        1. BoardService getVoardList() 메서드에 정렬로직 추가

    3. 게시글 개수만큼 번호가 나오도록 수정
        1. 현재는 각 페이지마자 1~10까지 반복
        2. 게시물번호 = 전체 게시물개수 - (현재 페이지번호 * 10[페이지당 게시글 수]) - 페이지당 인덱스
    ...
    4. Bootstrap 배지로 각 게시글마다 댓글개수 표시
        ...
        <img src="./image/sb016.png" width="600" height="350">
...
2. Spring Boot Security : 회원가입, 로그인 등을 손쉽게 개발하도록 도와주는 의존성 라이브러리
    ...
    2. 로그인 화면 및 H2 DB 사용불가
        - 기본 사용자 : user
        - 패스워드 : Spring Boot 로그에 표시 (ex: e2e2ba13-2e36-49b2-956f-c6739117e35b)
    ...
    5. 스프링 시큐리티 설정 (계속)
        1. SecurityConfig 클래스 내 filterChain 메서드에 CSRF 등 관련 설정 추기
...
6. 웹 html 에디터 적용
    ...
    2. Trumbowyg 적용
        ...
        <img src="./image/sb017.png" width="600">
...
1. 부트스트링 프리테마 NiceSchool로 변경
    ...
    4. 관리자(사용자)가 쉽게 데이터를 입력할 수 있는 화면 개발
        ...
        6. AdminController @PostMappint("/history/{id}") 메서드 생성
...
3. 공동 데이터 포털
    1. 회원가입 및 로그인
    2. [부산광역시 부산맛집정보](https://www.data.go.kr/data/15063472/openapi.do) 서비스 활용 신청

        <img src="/image/sb019.png" width="700"> 

    3. 마이페이지 > 데이터활용 > OpenAPI > 활용신청 현황
    4. 개발계정 상세보기
    ...
    6. 미리보기
        ...
        <img src="/image/sb020.png" width="600">
...
4. OpenAPI 호출 로직 구현
    ...
    2. controller.RestaurantController 생성
        - 기본 요청 RESTfull URL GetMapping 메서드 생성

    3. service.RestaurantService 생성 
        - fetchRestaurant() 메서드 작성
