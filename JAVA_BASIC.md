...
- 연산자는 거의 대부분의 언어에서 동일하게 사용
    ...
    - 논리연산 : &&, ||, !
    - 관계연산 : ==, >, >=, <, <=, !=
    - [Java문법실습](./day01/spring01/src/main/java/com/jm1109/spring01/Spring01Application.java)
...
- 조건문 : if문, switch문
    ...
    ```
    - [Java문법실습](./day01/spring02/src/main/java/com/jm1109/spring02/Spring02Application.java)
...
- 반복문 : for문, while문, do-while문
    ...
    - [Java문법실습](./day01/spring03/src/main/java/com/jm1109/spring03/Spring03Application.java)
...
- [Java문법실습](./day02/spring01/src/main/java/com/jm1109/spring01/Spring01Application.java)

- 반복제어 키워드 : break, continue
    - for문, while문 내에서 사용
    - break - 조건에 맞에 반복문을 탈출할 때
...
- 중간의 데이터를 추가하거나 삭제하려면 기존 데이터를 모두 이동해야 함
    ...
    타입[] 변수명 = [데이터 배열값, ...];
    타입[] 변수명 = new 타입[크기];     // 아무값 없이 초기화

    변수명[인덱스];     // 입력된 인덱스의 위치
    변수명[인덱스] = 값;     // 입력된 인덱스에 값 할당
    System.out.println(변수명[인덱스]); // 인덱스에 있는 값 출력
    ```

- 배열에 사용할 메서드
    - sort() : 배열 정렬
    - toString() : 배열 객체ID를 출력
    - asList() : 배열용 리스트로 변경(ArrayList)
...
- [Java문법실습](./day02/spring02/src/main/java/com/jm1109/spring02/Spring02Application.java)
...
- 메서드 오버로딩 - 동일한 메서드명, 다른 파라미터
- 가변 인자(파라미터) - 파라미터 개수가 동적일 때

- **static**
    - 소스코드 실행 후 클래스 인스턴스가 생성안됨
    - 이 상태에서 프로그램 엔트리포인트 메서드는 바로 실행되어야 함
    - static으로 생성하면 프로그램 실행 전부터 종료 직전까지 그냥 사용할 수 있는 메서드
...
- [Java문법실습](./day02/spring03/src/main/java/com/jm1109/spring03/Spring03Application.java)
...
- [Java문법실습](./day02/spring04/src/main/java/com/jm1109/spring04/Spring04Application.java)
...
- 특징
    ...
    - 부모클래스에서 불려받은 메서드를 수정할 수 있다. (메서드 오버라이딩 - 메서드 재정의)
    - 동일한 부모클래스를 상속받은 하위 자식클래스들은 타이빙 호환된다
...
- [Java문법실습](./day02/spring05/src/main/java/com/jm1109/spring05/Spring05Application.java)

#### 접근제어자
- 클래스, 변수, 메서드 등에 어떤 방식으로 접근할 건지를 정의하는 키워드
- 객체지향의 특징 중 캡슐화를 위해서 사용
- 접근제어자 분류
    - public : 접근 제한이 없음. 누구나 접근가능
    - private : 같은 클래스 내에서만 접근가능
    - protected : 같은 패키지 내 또는 다른 패키지의 자식 클래스에서만 접근가능
    - default : (키워드 생략가능) 같은 패키지 내에서만 접근가능
...
- 캡슐화를 제대로 하기위한 코딩방법
    ...
    - Getter : 값을 리턴. 포메팅 해서 리턴 가능, get이라는 접두어로 시작하는 메서드
        - get + 멤버변수명으로 메서드명 생성
    - Setter :  중요! 잘못된 값이 들어가지 않도록 막아주는 메서드
        - set + 멤버변수명(입력파라미터) 형태로 생성
        - 내부로직으로 입력값 필터링
    
    - 변수? 생성 후 단축키 : Ctrl + . > Generate getter and setter 선택
...
- 아무런 기능이 없고 단지 어떻게 구현을 해야한다는 규칙만 선언되어 있음
...
- [Java문법실습](./day03/spring01/src/main/java/com/jm1109/spring01/Spring01Application.java)
...
- [Java문법실습](./day03/spring02/src/main/java/com/jm1109/spring02/Spring02Application.java)
...
- [Java문법실습](./day03/spring04/src/main/java/com/jm1109/spring04/Spring04Application.java)
...
- [Java문법실습](./day04/spring01/src/main/java/com/jm1109/spring01/Spring01Application.java)


#### 제네릭
- 파이썬과 다르게 Java는 단일형만 배열이나 컬렉션에서 사용할 수 있음
- Object 타입으로 지정하면 무슨 형이든 다 할당 가능
- `다양한 타입의 객체들은 다루는 메서드나 컬렉션 클래스를 컴파이 ㄹ과정에서 안전하게 타입체크를 해주는 기능`
- 장점
    - 객체 생성시 개발자가 원하는 타입을 지정 가능
    - 타입 안정성 제공
    - 의도하지 않은 타입의 객체가 지장되는 것을 차단, 오류방지
    - 형변환의 번거로움이 없음
...
- T는 가변타입. String, int, double 등 다 지정 가능
- T, P, K, TP 등 가변타입의 명칭은 편하게 지정

- [Java문법실습](./day04/spring02/src/main/java/com/jm1109/spring02/Spring02Application.java)
...
- Map : 키-값 쌍의 데이터 집합. 순서X, 키중복X, 값중복O
...
- 리스트 인터페이스
    ...
    List<String> lsit = new ArrayList<>();      // 권장
    ```
...
- [Java문법실습](./day04/spring03/src/main/java/com/jm1109/spring03/Spring03Application.java)
...
#### 스트림 API
- 함수형 인터페이스로 데이터를 추상화하고 처리하는 데에 자주 사용되는 함수들을 정의해놓은 것
- 특징
    - 원본 데이터를 변경안함
    - 일회성
    - 내부 반복으로 작업처리
...
- 클래스, 메서드 바로 위에 작성. 코드와 설정을 관히라 수 있게 도와주는 역할
...
##### 2. @Deprecated
- 암프로 다음버전에서 삭제될 수 있음. 사용하지 말 것을 권유하는 체크

<img src="./image/sb009.png" width="700">

- 되도록이면 이 함수는 사용하지 말것

##### 3. @FunctionalInterfade
